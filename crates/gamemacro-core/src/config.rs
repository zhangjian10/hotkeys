...
pub struct HotkeyConfig {
    /// 修饰键序列（来自 ALL_MODIFIERS：Alt / Ctrl / Shift / Meta）。
    /// **保留用户给出的顺序**——查重 / 触发匹配都按数组本身做。
    /// 空数组 = 无修饰键（不推荐）。
    #[serde(default)]
...
impl HotkeyConfig {
    ...
    /// 形如 `"Ctrl+Shift::F1"`；modifiers 顺序保留，所以 (Ctrl,Shift) 与 (Shift,Ctrl)
    /// 视为不同条目（与用户在 GUI 录制时的体验一致）。
...
mod tests {
    ...
    #[test]
    fn hotkey_repeat_defaults_to_true() {
        let toml_str = r#"
[[profiles]]
name = "T"
window_keywords = ["x"]
...
"#;
        ...
        assert!(hk.repeat, "repeat should default to true");
...
"#;
    ...
    #[test]
    fn hotkey_modifiers_preserve_input_order() {
        // 用户输入顺序要被保留（不再排序、不再去重）
        let toml_str = r#"
[[profiles]]
name = "T"
window_keywords = ["x"]
...
"#;
    ...
    #[test]
    fn hotkey_combo_signature_preserves_modifier_order() {
        let a = HotkeyConfig {
        ...
        let b = HotkeyConfig {
            modifiers: vec!["Shift".into(), "Ctrl".into()],
            trigger_key: "F1".into(),
            input_string: String::new(),
            description: None,
            repeat: true,
        ...
        // 不同顺序 = 不同 signature（作为不同条目存在）
        assert_eq!(a.combo_signature(), "Ctrl+Shift::F1");
        assert_eq!(b.combo_signature(), "Shift+Ctrl::F1");
        assert_ne!(a.combo_signature(), b.combo_signature());
    }
